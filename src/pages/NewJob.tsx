...
// Arriving from /pendientes with a row already picked (see PendingJobs.tsx)
// — skip straight to the guided form for that flow/item instead of making
// the person pick the flow and find the row again.
...
export default function NewJob() {
  ...
  // Once a flow is picked, an OPERATIONAL user can work through its
  // CSV-loaded queue of pending jobs (see PendingQueuePanel) instead of
  // retyping each one — click a row there to pre-fill the guided form with
  // it; submitting removes it from the queue.
  const [activePending, setActivePending] = useState<PendingJobItem | null>(null);
  const [manualEntry, setManualEntry] = useState(false);

  // Arriving from /pendientes with a row already picked — jump straight to
  // the guided form instead of making the person pick the flow and find the
  // row again. Only consumed once: react-router keeps state on back/forward
  // navigation, so clear it after use to avoid re-triggering on a later visit.
  const location = useLocation();
  useEffect(() => {
    const incoming = location.state as IncomingPendingState | null;
    if (!incoming || !isOperational) return;
    setSelectedGrantedFlow(incoming.flow);
    setActivePending(incoming.item);
    ...
    if (!activePending && !manualEntry) {
      return (
        <div>
          ...
          <div className="stub p-6">
            <PendingQueuePanel
              flow={selectedGrantedFlow}
              onProcess={setActivePending}
